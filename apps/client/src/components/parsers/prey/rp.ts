...
import { EOI, BadToken } from '../pcx/rputil.js';
import { jsonTree as treeify } from 'https://deno.land/x/json_tree@latest/mod.ts';
...
class Tokenizer {
  ...
  cursor: number = 0;
  parsers: PCox<string>[];
  constructor(parsers: PCox<string>[]) {
...
class Language {
  input: string = '';
  tokenizer: Tokenizer;
  peek: Token = { type: token.equal, value: '=' };
  option: BuildOption;
...
const typedef = (d: PCox<string | string[] | any>, typeName: TokenType) =>
  d.map<string>((d) => ({
    type: typeName,
    out:
